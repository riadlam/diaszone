...
class WeeklyPassUiAndOrderTest extends TestCase
{
    ...
    public function test_weekly_pass_pack_shows_3x_badge_and_creates_order_with_quantity()
    {
        ...
        // Perform POST to create the order (checkout requires authentication)
        $response = $this->actingAs(User::factory()->create())->postJson(route('api.orders.create'), [
            'cart_items' => [
        ...
        $this->assertEquals(3, (int) $order->orderItems->sum('quantity'), 'Order items quantity mismatch: '.print_r($order->toArray(), true));
