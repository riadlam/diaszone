    ...
    @if(isset($availableRegions) && $availableRegions->isNotEmpty())
        <div class="mb-6 hidden lg:block bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-100">
            <div class="flex flex-wrap gap-2 items-center">
                ...
                <button type="button" 
                        class="region-filter-btn px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 bg-purple-600 text-white hover:bg-purple-700 hover:shadow-lg active:scale-95 shadow-md border-2 border-purple-700" 
                        data-region="all">
                    All Regions
                </button>
                @foreach($availableRegions as $region)
                    <button type="button" 
                            class="region-filter-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 bg-white text-gray-700 hover:bg-gray-50 hover:shadow-md active:scale-95 border-2 border-gray-300 hover:border-purple-400" 
            ...
            <div class="diamond-pack-item-wrapper" data-pack-region="{{ $pack->region ?? '' }}">
                ...
                @endforeach
                    ...
                    <div class="diamond-pack-item-wrapper">
                        ...
                        <div class="SKU_type bg-white border-2 border-gray-200 rounded-lg p-4 hover:border-purple-500 transition-all relative" data-pack-wrapper="{{ $pack->id }}">
                            <!-- Quantity Selector (visible when checked) -->
                            <div class="quantity-selector absolute top-2 right-2 hidden">
                                <div class="flex items-center space-x-2 bg-purple-100 rounded-lg px-2 py-1">
                                    <button type="button" class="decrease-qty text-purple-600 hover:text-purple-800 font-bold" data-pack-id="{{ $pack->id }}">-</button>
                                    <span class="qty-display text-purple-700 font-semibold min-w-[30px] text-center" data-pack-id="{{ $pack->id }}">1</span>
                                    <button type="button" class="increase-qty text-purple-600 hover:text-purple-800 font-bold" data-pack-id="{{ $pack->id }}">+</button>
                                </div>
                            </div>
                    ...
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1">
                            ...
                            @endphp
                            @if($pack->diamonds == 0 && $pack->membership_name)
                                <p class="text-sm font-medium text-gray-700">{{ $pack->membership_name }}</p>
                            @else
                                @if(stripos($pack->name, 'Weekly Diamond Pass') !== false || stripos($pack->name, 'Event Topup') !== false)
                                    <p class="text-sm font-medium text-gray-700">{{ __('game.weekly_diamond_pass') }}</p>
                                @elseif(stripos($pack->name, 'Twilight Pass') !== false)
                                    <p class="text-sm font-medium text-gray-700">{{ __('game.twilight_pass') }}</p>
                                @else
                                    @if($packQuantity > 1)
                                        <p class="text-sm font-medium text-gray-700">{{ $packQuantity }}x {{ $pack->diamonds }} {{ $currencyName }}</p>
                                    @else
                                        <p class="text-sm font-medium text-gray-700">{{ $pack->diamonds }} {{ $currencyName }}</p>
                                    @endif
                                @endif
                            @endif
                            @if($pack->bonus_diamonds > 0)
                                <p class="text-xs text-purple-600 mt-1">+ {{ $pack->bonus_diamonds }} {{ __('game.bonus') }} {{ $currencyName }}</p>
                            @endif
                            @if($pack->discount_percentage > 0)
                                <span class="inline-block bg-red-100 text-red-800 text-xs font-semibold px-2 py-1 rounded mt-1">
                                    -{{ number_format($pack->discount_percentage, 0) }}%
                                </span>
                            @endif
                        </div>
                    </div>
...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set initial state - show all packs
        ...
        function filterPacks(region) {
            ...
            regionButtons.forEach(btn => {
                ...
                if (btnRegion === region) {
                    btn.classList.remove('bg-white', 'bg-gray-200', 'text-gray-700', 'hover:bg-gray-50', 'hover:bg-gray-300', 'border-gray-300', 'hover:border-purple-400', 'shadow-sm');
